...
import { LayoutDashboard, Wallet, Receipt, Tags, Users, BarChart3, PanelLeftClose, PanelLeftOpen, LogOut } from 'lucide-react';
...
export function Sidebar() {
  ...
  return (
    ...
    >
      ...
      <button onClick={() => void signOut()} className="mx-3 mb-2 flex items-center gap-3 px-3 py-3 rounded-xl text-muted-foreground hover:bg-white/5 hover:text-foreground transition-colors" title="Sign out">
        <LogOut size={22} className="shrink-0" />
        {!collapsed && <span className="font-medium truncate">{session?.user.email}</span>}
      </button>
